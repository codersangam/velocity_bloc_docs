...
### Step 5
Let' create the function when we can increase or decrease the number. The increment function is taking one parameter `data` which is of int type. And ofcourse, we want to increse the number by one eaach time so `data++` and we need to inform that the data has been increased by one so `incrementBloc.onUpdateData(data)`
